...
// Constants
const ALLOWANCE_QUERY = "id, priest_id, nurs_care_insu, month, profiles!salary_priest_id_fkey(full_name, email)";

export default function AdminNursingInsurance() {
  ...
  const [entries, setEntries] = useState<NursingInsuranceRow[]>([]);
  ...
  // Load nursing insurance data
  const loadNursingInsuranceData = useCallback(async () => {
    ...
    const insuranceData = (data ?? []) as unknown as NursingInsuranceRow[];
    setEntries(insuranceData);
    
    // Calculate total
    const total = insuranceData.reduce((sum, item) => sum + (item.nurs_care_insu || 0), 0);
  ...
  useEffect(() => {
    ...
    loadNursingInsuranceData();
  }, [startMonth, endMonth, loadNursingInsuranceData, user, loading]);
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl md:text-2xl font-semibold text-gray-800 mb-3">
        {t("adminNursingCareInsurance.title")}
      </h1>
      ...
      <div className="flex flex-col lg:flex-row gap-2">
          ...
          <div className="overflow-auto rounded-lg max-h-[calc(100vh-21rem)] thin-scroll">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-3 py-2 text-left whitespace-nowrap">{t("adminNursingCareInsurance.priestColumn")}</th>
                  <th className="px-3 py-2 text-left whitespace-nowrap">{t("adminNursingCareInsurance.monthColumn")}</th>
                  <th className="px-3 py-2 text-right whitespace-nowrap">{t("adminNursingCareInsurance.amountColumn")}</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((item) => (
                  <tr key={item.id} className="border-t border-gray-100">
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">€ {item.nurs_care_insu}</td>
                  </tr>
                ))}
                {!entries.length && (
                  <tr>
                    <td colSpan={3} className="px-3 py-6 text-center text-gray-500">
                      {t("adminNursingCareInsurance.noInsuranceEntries")}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="w-full lg:w-1/3 h-fit bg-white border border-gray-200 rounded-lg">
          <div className="p-2 border-b border-gray-200">
            <h2 className="font-semibold">{t("adminNursingCareInsurance.nursingCareInsuranceSummary")}</h2>
          </div>
