...
//   createUser();
  ...
  const fetchSummary = useCallback(async () => {
    ...
    const [
      ...
      supabase.from("loans").select("id").gte("closed_on", today),
    ...
    const totalRent = rentSummary?.data
      ? rentSummary.data.reduce(
          (acc: any, curr: any) => acc + (curr.total_payout || 0),
          0,
        )
      : 0;

    const totalSalary = salarySummary?.data
      ? salarySummary.data.reduce(
          (acc: any, curr: any) => acc + (curr.total_payout || 0),
          0,
        )
      : 0;
    ...
    const insuranceDescription =
      insuranceTypes.length > 0
        ? insuranceTypes
            .map((type: number) => insuranceTypeNames[type] || `Type ${type}`)
            .join(", ")
    ...
    const lastTransfersTotal = lastTransfers?.data
      ? lastTransfers.data.reduce(
          (acc: any, curr: any) => acc + (curr.amount || 0),
          0,
        )
      : 0;
    const lastTransfersDescription =
      lastTransfers?.data && lastTransfers.data.length > 0
        ? lastTransfers.data
            .map((t: any) => t.notes || "No description")
            .filter((d: string) => d !== "No description")
            .join(", ") || "No description"
        : "No transfers";

    // Last Donation Received
    const lastDonationAmount =
      lastDonation?.data && lastDonation.data.length > 0
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500">
                {t("adminDashboard.totalSalary")} (This Month)
              </p>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500">
                {t("adminDashboard.totalInsurancePaid")} (This Month)
              </p>
              <p className="text-xl font-semibold mt-1">
                € {statusCards?.totalInsuranceThisMonth || "0.00"}
              </p>
              <p className="text-xs text-gray-400 mt-1">
                {statusCards?.insuranceDescription || "No insurance payments"}
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500">
                {t("adminDashboard.totalRentPaid")} (This Month)
              </p>
              <p className="text-xl font-semibold mt-1">
                € {statusCards?.totalRentThisMonth || "0.00"}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500">
                {t("adminDashboard.totalLoansIssued")} (This Month)
              </p>
              <p className="text-xl font-semibold mt-1">
                {" "}
                {statusCards?.totalLoansIssuedThisMonth || 0}
              </p>
            </div>
            {/* 5. Number of Active Loans */}
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500">
                {t("adminDashboard.numberOfActiveLoans")}
              </p>
              <p className="text-xl font-semibold mt-1">
                {statusCards?.numberOfActiveLoans || 0}
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500">
                {t("adminDashboard.lastInternationalTransfers")}
              </p>
              <p className="text-xl font-semibold mt-1">
                € {statusCards?.lastInternationalTransfersTotal || "0.00"}
              </p>
              <p
                className="text-xs text-gray-400 mt-1 truncate"
                title={statusCards?.lastInternationalTransfersDescription}
              >
                {statusCards?.lastInternationalTransfersDescription ||
                  "No transfers"}
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              ...
              <p className="text-xl font-semibold mt-1">
                € {statusCards?.lastDonationReceived || "0.00"}
              </p>
              {statusCards?.lastDonationDate && (
                <p className="text-xs text-gray-400 mt-1">
                  {new Date(statusCards.lastDonationDate).toLocaleDateString(
                    undefined,
                    {
                      day: "numeric",
                      month: "short",
