...
// Constants
const ALLOWANCE_QUERY = "id, priest_id, others, month, profiles!salary_priest_id_fkey(full_name, email)";

export default function AdminOtherExpenses() {
  ...
  const [entries, setEntries] = useState<OtherExpensesRow[]>([]);
  ...
  const loadOtherExpensesData = useCallback(async () => {
    ...
    const otherExpensesData = (data ?? []) as unknown as OtherExpensesRow[];
    setEntries(otherExpensesData);
    
    // Calculate total
    const total = otherExpensesData.reduce((sum, item) => sum + (item.others || 0), 0);
  ...
  }, [startMonth, endMonth, loadOtherExpensesData, user, loading]);
      ...
      <div className="flex flex-col lg:flex-row gap-2">
          ...
          <div className="overflow-auto rounded-lg max-h-[calc(100vh-21rem)] thin-scroll">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-3 py-2 text-left whitespace-nowrap">{t("adminOtherExpenses.priestColumn")}</th>
                  <th className="px-3 py-2 text-left whitespace-nowrap">{t("adminOtherExpenses.monthColumn")}</th>
                  <th className="px-3 py-2 text-right whitespace-nowrap">{t("adminOtherExpenses.amountColumn")}</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((item) => (
                  <tr key={item.id} className="border-t border-gray-100">
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">€ {item.others}</td>
                  </tr>
                ))}
                {!entries.length && (
                  <tr>
                    <td colSpan={3} className="px-3 py-6 text-center text-gray-500">
                      {t("adminOtherExpenses.noOtherExpensesEntries")}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="w-full lg:w-1/3 h-fit bg-white border border-gray-200 rounded-lg">
          <div className="p-2 border-b border-gray-200">
            <h2 className="font-semibold">{t("adminOtherExpenses.summary")}</h2>
          </div>
