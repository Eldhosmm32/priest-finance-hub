...
// Constants
const ALLOWANCE_QUERY = "id, priest_id, km_alnce, month, profiles!salary_priest_id_fkey(full_name, email)";
...
export default function AdminKmAllowance() {
  ...
  const [entries, setEntries] = useState<KmAllowanceRow[]>([]);
  ...
  const loadKmAllowanceData = useCallback(async () => {
    ...
    const allowanceData = (data ?? []) as unknown as KmAllowanceRow[];
    setEntries(allowanceData);
    
    // Calculate total
    const total = allowanceData.reduce((sum, item) => sum + (item.km_alnce || 0), 0);
              ...
              <tbody>
                {entries.map((item) => (
                  <tr key={item.id} className="border-t border-gray-100">
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">€ {item.km_alnce}</td>
                  </tr>
                ))}
                {!entries.length && (
                  <tr>
